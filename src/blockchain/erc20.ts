...
const get_contract_signer_private_key = (blockchain: any, contract_address: string, private_key:string) => {
    // A Signer from a private key
...
export const Erc20 = {
    ...
    transfer_private_key: async (blockchain: any, contract_address: string,private_key:string, recipient: string, amount: string) => {
        let contractWithSigner = get_contract_signer_private_key(blockchain, contract_address,private_key);
        let tx = await contractWithSigner.transfer(recipient, amount);
        logger.info(tx.hash);
        return tx.hash;
    },
